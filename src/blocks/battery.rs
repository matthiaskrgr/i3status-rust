...
impl Block for Battery {
    fn update(&mut self) -> Result<Option<Duration>> {
        ...
        if file_exists(&format!("{}capacity", self.device_path)) {
            current_percentage =
                match read_file("battery", &format!("{}capacity", self.device_path))?
                    .parse::<u64>()
        ...
        } else if file_exists(&format!("{}energy_full", self.device_path))
            && file_exists(&format!("{}energy_now", self.device_path))
        {
            // We only need to read max_charge once, shouldn't change
            if self.max_charge == 0 {
        ...
        let (hours, minutes) = if power_now > 0 && energy_now > 0 {
            if state == "Discharging" {
                ...
                let m = ((((energy_full as f64 - energy_now as f64) / power_now as f64) - h as f64)
                    * 60.0) as u64;
                (h, m)
            } else {
                (0, 0)
